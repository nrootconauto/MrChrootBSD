...
char *fd_cache_none = "";
static size_t RoundUp(int fd) { return (fd & ~127) + 128 * 2; }
void FDCacheSet(CFDCache *c, int fd, char *v) {
  ...
  if (fd >= c->size) {
    ...
    if (c->values)
      memcpy(new, c->values, sizeof(char *) * c->size);
    c->size = RoundUp(fd);
    free(c->values);
    c->values = new;
  }
  if (FD_CACHE_NOT_FILE != c->values[fd])
    free(c->values[fd]);
  if (v == FD_CACHE_NOT_FILE)
    c->values[fd] = FD_CACHE_NOT_FILE;
  else
