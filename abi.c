...
int64_t ABIGetReturn(pid_t pid,char *failed) {
	struct reg regs;
	ptrace(PT_GETREGS,pid,&regs,(uint64_t)&regs);
...
void ABISetReturn(pid_t pid,int64_t r,char failed) {
	...
	ptrace(PT_SETREGS,pid,&regs,(uint64_t)&regs);
}
...
int64_t ABIGetArg(pid_t pid,int64_t arg) {
	...
	switch(arg) {
		...
		default:
			...
			return (ptrace(PT_READ_D,pid,(char*)stk_ptr,0)&0xffFFffFFul)|(ptrace(PT_READ_D,pid,(char*)stk_ptr+4,0)<<32ul);
	}
}
void ABISetArg(pid_t pid,int64_t arg,uint64_t val) {
	...
	switch(arg) {
		...
		default:
			stk_ptr=regs.r_rsp+8*(arg-6);
			ptrace(PT_WRITE_D,pid,(char*)stk_ptr,val&0xffFFffFFul);
			ptrace(PT_WRITE_D,pid,(char*)stk_ptr+4,val>>32ul);
			break;
	}
	if(arg<=5)
		ptrace(PT_SETREGS,pid,&regs,(uint64_t)&regs);
